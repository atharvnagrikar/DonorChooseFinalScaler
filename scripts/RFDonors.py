...
df_text=pd.read_csv("/email/dsml/df_textUpdated.csv")
# replacing processed title and essay with pre-processed respectively
preprocessed_data_df["essay"]=df_text["essay_message"]
preprocessed_data_df["project_title"]=df_text["essay_message"]
...
# classifier = XGBClassifier(learning_rate =0.01, n_estimators=1000,
#     max_depth=3, min_child_weight=5, gamma=0.4,subsample=0.55,colsample_bytree=0.85,
#     objective= 'binary:logistic', nthread=4,scale_pos_weight=0.17999999)
# Fit the clas  sifier to the preprocessed training data
classifier.fit(x_train_onehot, y_train)
pickle.dump(classifier, open("/email/dsml/models/RFClassifier.pkl", "wb"))
...
# rf_best.fit(x_train_onehot,y_train)

# # Make predictions on the test set
# y_pred = rf_best.predict(x_test_onehot)
